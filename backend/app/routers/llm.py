...
async def call_llm(
    body: LLMRequest,
    _: str = Depends(get_current_user),  # 認証のみ確認、user_idは不要
) -> LLMResponse:
    """Gemini 2.5 Flashへのリレー。replyとtitleを並列生成して返す"""
    if not body.userText:
        raise HTTPException(status_code=400, detail="userTextが必要です")

    # Gemini形式の会話履歴に変換（user/assistantをuser/modelに変換）
    history = [
        {
